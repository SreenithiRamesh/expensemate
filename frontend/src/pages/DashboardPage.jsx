...
// Replace with real API data during M32.
    ...
    spendingSeries: {
        weekly: {
            ...
            points: [
                ...
                { label: 'Thu', amount: 1450 },
                { label: 'Fri', amount: 400 },
                { label: 'Sat', amount: 600 },
                { label: 'Sun', amount: 500 },
            ],
            categories: [
                { label: 'Food & dining', value: 1240, color: '#159FA4' },
                { label: 'Travel', value: 700, color: '#3E8EF7' },
                { label: 'Shopping', value: 1450, color: '#E9B949' },
                { label: 'Bills', value: 1000, color: '#2F9E6F' },
            ],
        },
        ...
        monthly: {
            ...
            points: [
                { label: 'Wk 1', amount: 4230 },
                { label: 'Wk 2', amount: 5110 },
                { label: 'Wk 3', amount: 4720 },
                { label: 'Wk 4', amount: 4390 },
            ],
            categories: [
...
function DashboardPage() {
    ...
    const monthlySpending = monthlyPoints.reduce(
        (total, point) => total + point.amount,
        0,
    )

    const remainingBudget =
        dashboardPreviewData.monthlyBudget - monthlySpending
    ...
    function handleQuickAction(actionKey) {
        // Wire to real routes/forms in the corresponding milestones.
        ...
        <div className="space-y-6">
            ...
            >
                <SummaryCard
                    ...
                    icon={Wallet}
                    variant="teal"
                    sparklineData={monthlyPoints.map(
                        (point) => point.amount,
                    )}
                />
                ...
                <SummaryCard
                    title="You are owed"
                    value={formatCurrency(dashboardPreviewData.owedToUser)}
                    change="2 groups"
                    trend="up"
                    icon={ArrowUpCircle}
                    variant="green"
                ...
                <SummaryCard
                    title="You owe"
                    value={formatCurrency(dashboardPreviewData.userOwes)}
                    change="1 group"
                    trend="down"
            ...
            <section className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
                <SpendingChart
                    series={dashboardPreviewData.spendingSeries}
                    monthlyBudget={dashboardPreviewData.monthlyBudget}
            ...
            <section className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
                <RecentTransactions
                    ...
                    onViewAll={() =>
                        handleQuickAction('view-all-transactions')
                    }
                />

                <BudgetProgress
                    budget={dashboardPreviewData.monthlyBudget}
                    used={monthlySpending}
                    categories={dashboardPreviewData.budgetCategories}
                />
            </section>
